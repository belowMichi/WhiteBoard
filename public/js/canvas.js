...
$("#container").append(fontTip);//添加文本框输入
...
function beginRecord(){
    ...
    jq_canvas_tmp.one("click",function(evt){

       var btnPlace = getMousePos(canvas, evt);
        ctx_tmp.strokeRect(btnPlace.x, btnPlace.y,10, 10);
        recordBtn(btnPlace);
    });
...
//                clearContext(ctx_tmp);
            ctx_tmp.beginPath();
            ctx_tmp.moveTo(PositionArr[pode
            int].x, PositionArr[point].y);
...
var mm = function (evt) {
    if (token == "rectangle") {
        if(isDrawing)
        {
            ...
            ctx_tmp.strokeRect(begin.x, begin.y,end.x-begin.x, end.y-begin.y);
    ...
    if (token == "text") {
        if(isDrawing)
        {
            ...
            fontTip.show();
            fontTip.css({left:begin.x,top:begin.y});
            fontTip.width(now.x-begin.x);
            fontTip.height(now.y-begin.y);
        }
    }
    if (token == "line") {
        if (isDrawing) {
...
//        ctx_tmp.lineWidth = size;
    ...
    if (token == "text") {
        ...
        begin = getMousePos(canvas, evt);
        //var val=window.prompt("haha ");
        //ctx_tmp.fillText(val, begin.x, begin.y);
    }
    if (token == "line") {
        ctx.beginPath();
        ctx_tmp.beginPath();
...
var mu = function (evt) {
    ...
    if (token == "rectangle") {
        ...
        ctx.strokeRect(begin.x, begin.y,end.x-begin.x, end.y-begin.y);
